...
const cookie__namespace = /*#__PURE__*/_interopNamespace(cookie);
...
exports.PrismicPreviewState = void 0;
...
})(exports.PrismicPreviewState || (exports.PrismicPreviewState = {}));
exports.PrismicContextActionType = void 0;
...
})(exports.PrismicContextActionType || (exports.PrismicContextActionType = {}));
const contextReducer = (state, action) => {
  switch (action.type) {
    case exports.PrismicContextActionType.SetActiveRepositoryName: {
      return {
        ...state,
        activeRepositoryName: action.payload.repositoryName
      };
    }
    case exports.PrismicContextActionType.SetupRuntime: {
      const runtime = gatsbyPrismic__namespace.createRuntime({
    ...
    case exports.PrismicContextActionType.RegisterDocuments: {
    ...
    case exports.PrismicContextActionType.ImportTypePaths: {
      const runtime = state.runtimeStore[action.payload.repositoryName];
      if (runtime) {
    ...
    case exports.PrismicContextActionType.SetAccessToken: {
    ...
    case exports.PrismicContextActionType.StartResolving: {
      return {
        ...state,
        previewState: exports.PrismicPreviewState.RESOLVING
      };
    }
    case exports.PrismicContextActionType.Resolved: {
      return {
        ...state,
        previewState: exports.PrismicPreviewState.RESOLVED,
        resolvedPath: action.payload.path
      };
    }
    case exports.PrismicContextActionType.StartBootstrapping: {
      return {
        ...state,
        previewState: exports.PrismicPreviewState.BOOTSTRAPPING,
        isBootstrapped: false
      };
    }
    case exports.PrismicContextActionType.Bootstrapped: {
      return {
        ...state,
        previewState: exports.PrismicPreviewState.ACTIVE,
        isBootstrapped: true
      };
    }
    case exports.PrismicContextActionType.Failed: {
      return {
        ...state,
        previewState: exports.PrismicPreviewState.FAILED,
        error: action.payload.error
      };
    }
    case exports.PrismicContextActionType.NotAPreview: {
      return {
        ...state,
        previewState: exports.PrismicPreviewState.NOT_PREVIEW
      };
    }
    case exports.PrismicContextActionType.PromptForAccessToken: {
      return {
        ...state,
        previewState: exports.PrismicPreviewState.PROMPT_FOR_ACCESS_TOKEN
      };
    }
    case exports.PrismicContextActionType.GoToIdle: {
      return {
        ...state,
        previewState: exports.PrismicPreviewState.IDLE
      };
    }
  }
};
...
const createInitialState = (repositoryConfigs = defaultInitialState.repositoryConfigs) => {
  ...
  const injectedPluginOptionsStore = repositoryNames.reduce((acc, repositoryName) => {
    ...
    const persistedAccessToken = cookie__namespace.get(persistedAccessTokenCookieName);
...
const PrismicContext = React__namespace.createContext(defaultContextValue);
const PrismicPreviewProvider = ({
  repositoryConfigs,
  children
}) => {
  const initialState = createInitialState(repositoryConfigs);
  const reducerTuple = React__namespace.useReducer(contextReducer, initialState);
...
const findAndReplacePreviewables = (runtime, nodeOrLeaf) => {
  if (isPlainObject(nodeOrLeaf)) {
    ...
    const nodeId = nodeOrLeaf[gatsbyPrismic__namespace.PREVIEWABLE_NODE_ID_FIELD];
...
const useMergePrismicPreviewData = (staticData, config = { skip: false }) => {
  ...
  return React__namespace.useMemo(() => {
    ...
    if (!config.skip && runtime && state.previewState === exports.PrismicPreviewState.ACTIVE) {
...
const usePrismicPreviewAccessToken = (repositoryName) => {
  ...
  const setAccessToken = React__namespace.useCallback((accessToken, remember = true) => {
    ...
    contextDispatch({
      type: exports.PrismicContextActionType.SetAccessToken,
      payload: { repositoryName, accessToken }
    });
    if (remember) {
      cookie__namespace.set(cookieName, accessToken);
    }
  }, [cookieName, contextDispatch, repositoryName]);
  const removeAccessTokenCookie = React__namespace.useCallback(() => {
    if (!cookieName) {
    ...
    cookie__namespace.remove(cookieName);
  }, [cookieName]);
  return React__namespace.useMemo(() => {
    var _a;
    return [
...
const extractFirstSubdomain = (host) => O__namespace.fromNullable(host.split(".")[0]);
const parseObjectRef = (previewRef) => {
  try {
    const parsed = JSON.parse(previewRef);
    const keys = Object.keys(parsed);
    const domainKey = keys.find((key) => /\.prismic\.io$/.test(key));
    return domainKey ? extractFirstSubdomain(domainKey) : O__namespace.none;
  } catch (e) {
    return O__namespace.none;
  }
};
...
const extractPreviewRefRepositoryName = (previewRef) => {
  ...
  if (O__namespace.isSome(fromObjectRef)) {
...
const fetchTypePaths = async (config) => {
  ...
  const url = gatsby__namespace.withAssetPrefix(`/static/${filename}`);
  ...
  try {
    ...
    return E__namespace.right(text);
  } catch (error) {
...
const usePrismicPreviewBootstrap = (repositoryConfigs = [], config = {}) => {
  const [contextState, contextDispatch] = usePrismicPreviewContext();
  const contextStateRef = React__namespace.useRef(contextState);
  React__namespace.useEffect(() => {
    contextStateRef.current = contextState;
  }, [contextState]);
  return React__namespace.useCallback(async () => {
    var _a;
    if (contextStateRef.current.previewState !== exports.PrismicPreviewState.IDLE && contextStateRef.current.previewState !== exports.PrismicPreviewState.RESOLVED || contextStateRef.current.isBootstrapped) {
      return;
    }
    const previewRef = cookie__namespace.get(prismic__namespace.cookie.preview);
    const repositoryName = previewRef ? extractPreviewRefRepositoryName(previewRef) : O__namespace.none;
    if (O__namespace.isNone(repositoryName)) {
      return contextDispatch({
        type: exports.PrismicContextActionType.NotAPreview
      });
    }
    contextDispatch({
      type: exports.PrismicContextActionType.SetActiveRepositoryName,
    ...
    if (!repositoryConfig) {
      return contextDispatch({
        type: exports.PrismicContextActionType.Failed,
        payload: {
    ...
    contextDispatch({
      type: exports.PrismicContextActionType.SetupRuntime,
      payload: {
    ...
    contextDispatch({
      type: exports.PrismicContextActionType.StartBootstrapping
    });
    const typePaths = await fetchTypePaths({
      repositoryName: repositoryName.value,
      fetch: config.fetch
    });
    if (E__namespace.isLeft(typePaths)) {
      return contextDispatch({
        type: exports.PrismicContextActionType.Failed,
    ...
    contextDispatch({
      type: exports.PrismicContextActionType.ImportTypePaths,
      payload: {
        repositoryName: repositoryName.value,
        typePathsExport: typePaths.right
      }
    });
    const endpoint = (_a = repositoryPluginOptions.apiEndpoint) != null ? _a : prismic__namespace.getEndpoint(repositoryName.value);
    ...
    try {
      ...
      if (error instanceof prismic__namespace.ForbiddenError && repositoryPluginOptions.promptForAccessToken) {
        return contextDispatch({
          type: exports.PrismicContextActionType.PromptForAccessToken
        });
      } else {
        return contextDispatch({
          type: exports.PrismicContextActionType.Failed,
    ...
    contextDispatch({
      type: exports.PrismicContextActionType.RegisterDocuments,
      payload: {
        repositoryName: repositoryName.value,
    ...
    contextDispatch({
      type: exports.PrismicContextActionType.Bootstrapped
    });
  }, [
    repositoryConfigs,
    contextState.repositoryConfigs,
...
const getURLSearchParam = (key) => {
  ...
  return O__namespace.fromNullable(params.get(key));
};
...
const usePrismicPreviewResolver = (repositoryConfigs = [], config = {}) => {
  ...
  return React__namespace.useCallback(async () => {
    var _a;
    if (contextStateRef.current.previewState !== exports.PrismicPreviewState.IDLE) {
      return;
    }
    const previewRef = cookie__namespace.get(prismic__namespace.cookie.preview);
    const documentId = getURLSearchParam("documentId");
    const repositoryName = previewRef ? extractPreviewRefRepositoryName(previewRef) : O__namespace.none;
    if (O__namespace.isNone(documentId) || O__namespace.isNone(repositoryName)) {
      return contextDispatch({
        type: exports.PrismicContextActionType.NotAPreview
      });
    }
    ...
    contextDispatch({
      type: exports.PrismicContextActionType.StartResolving
    });
    const endpoint = (_a = repositoryPluginOptions.apiEndpoint) != null ? _a : prismic__namespace.getEndpoint(repositoryName.value);
    const client = prismic__namespace.createClient(endpoint, {
      accessToken: repositoryPluginOptions.accessToken,
    ...
    contextDispatch({
      type: exports.PrismicContextActionType.Resolved,
      payload: { path }
    });
  }, [
    contextDispatch,
...
const styles = css_248z;

const Root = ({ children }) => {
  return /* @__PURE__ */ React__namespace.createElement(root__default["default"].div, null, /* @__PURE__ */ React__namespace.createElement("div", {
    className: "root"
  }, children), /* @__PURE__ */ React__namespace.createElement("style", {
    type: "text/css"
  }, styles));
};

const CloseSVG = (props) => /* @__PURE__ */ React__namespace.createElement("svg", {
  viewBox: "0 0 18 18",
...
const Modal = ({
  ...
  return /* @__PURE__ */ React__namespace.createElement(dialog.DialogOverlay, {
    isOpen,
    onDismiss,
    dangerouslyBypassFocusLock: true
  }, /* @__PURE__ */ React__namespace.createElement(dialog.DialogContent, {
    "aria-label": ariaLabel
  }, /* @__PURE__ */ React__namespace.createElement(Root, null, /* @__PURE__ */ React__namespace.createElement("div", {
    ...
    className: clsx__default["default"]("rounded-lg shadow-lg px-7 py-8 relative sm:px-10", variant === "base" && "bg-white text-slate-30", variant === "red" && "bg-red-40 text-white"),
    "data-gatsby-plugin-prismic-previews-repository-name": repositoryName
  }, /* @__PURE__ */ React__namespace.createElement("div", {
    className: "grid gap-7"
  }, /* @__PURE__ */ React__namespace.createElement(PrismicLogo, {
    fillWhite: variant === "red",
    className: "block mx-auto w-11 h-11"
  }), /* @__PURE__ */ React__namespace.createElement("div", null, children)), /* @__PURE__ */ React__namespace.createElement("button", {
    className: clsx__default["default"]("absolute top-5 right-5 transition  sm:top-6 sm:right-6 p-2 -m-2", variant === "base" && "text-slate-90 hover:text-slate-60 focus:text-slate-60", variant === "red" && "text-red-80 hover:text-white focus:text-white"),
    onClick: onDismiss
  }, /* @__PURE__ */ React__namespace.createElement("span", {
    className: "sr-only"
  }, "Close modal"), /* @__PURE__ */ React__namespace.createElement(CloseSVG, {
    className: "w-5 h-5"
  }))))))));
};
...
const Text = ({
  ...
  return /* @__PURE__ */ React__namespace.createElement(Element, {
    ...restProps,
    className: clsx__default["default"](fontFamilyClassName != null ? fontFamilyClassName : variant.fontFamilyClassName, fontSizeClassName != null ? fontSizeClassName : variant.fontSizeClassName, leadingClassName != null ? leadingClassName : variant.leadingClassName, trackingClassName != null ? trackingClassName : variant.trackingClassName, className)
  });
};
...
const ModalAccessToken = ({
  ...
  const [ephemeralAccessToken, setEphemeralAccessToken] = React__namespace.useState(initialAccessToken);
  React__namespace.useEffect(() => {
  ...
  return /* @__PURE__ */ React__namespace.createElement(Modal, {
  ...
  }, "Enter your Prismic ", /* @__PURE__ */ React__namespace.createElement("br", {
    className: "sm:hidden"
  }), "access token"), /* @__PURE__ */ React__namespace.createElement(Text, {
    variant: "sans-12-14",
    className: "text-center"
  }, "An access token is required to view this preview.", /* @__PURE__ */ React__namespace.createElement("br", null), "Repository Name:", " ", /* @__PURE__ */ React__namespace.createElement("strong", {
    className: "font-medium text-slate-10"
  }, repositoryName))), /* @__PURE__ */ React__namespace.createElement("form", {
    onSubmit,
    className: "grid gap-5"
    ...
    className: "sr-only"
  }, "Access token"), /* @__PURE__ */ React__namespace.createElement("input", {
    name: "access-token",
    ...
    className: clsx__default["default"]("border rounded px-5 py-3 block font-mono text-base leading-none w-full", !ephemeralAccessToken && "text-center", state === "IDLE" && "border-slate-90 bg-slate-95 text-slate-30 placeholder-slate-70", state === "INCORRECT" && "border-red-40 bg-red-95 text-red-40 placeholder-red-80")
  }), state === "INCORRECT" && /* @__PURE__ */ React__namespace.createElement(Text, {
    variant: "sans-12-14",
    className: "text-red-40 font-semibold text-center"
  }, "Incorrect token")), /* @__PURE__ */ React__namespace.createElement("ul", {
  ...
  }, "Cancel"))), /* @__PURE__ */ React__namespace.createElement("li", {
    className: "pl-4 pt-4 pointer-events-auto"
  }, /* @__PURE__ */ React__namespace.createElement(Button, {
    variant: "purple",
    type: "submit"
    ...
    variant: "sans-14",
    className: "font-semibold"
  }, "Continue"))))), /* @__PURE__ */ React__namespace.createElement(Text, {
    variant: "sans-12",
    className: "text-center"
  }, "Not sure what your access token is? ", /* @__PURE__ */ React__namespace.createElement("br", {
    className: "sm:hidden"
...
const ModalError = ({
  ...
  }, "Error"), /* @__PURE__ */ React__namespace.createElement(Text, {
    variant: "sans-12-14",
    className: "text-center"
  }, "The preview could not be loaded.")), errorMessage && /* @__PURE__ */ React__namespace.createElement(Text, {
    variant: "mono-20",
    className: "text-center my-2"
  }, errorMessage), /* @__PURE__ */ React__namespace.createElement(Button, {
  ...
  }, "Cancel Preview")), /* @__PURE__ */ React__namespace.createElement("dl", {
    className: "text-red-80 flex flex-wrap -mt-5 -ml-5 justify-center"
  }, /* @__PURE__ */ React__namespace.createElement("div", {
    className: "grid gap-2 pl-5 pt-5 grid-flow-col"
  }, /* @__PURE__ */ React__namespace.createElement("dt", null, /* @__PURE__ */ React__namespace.createElement(Text, {
    variant: "sans-12",
    className: "font-semibold"
  }, "Repository")), /* @__PURE__ */ React__namespace.createElement("dd", null, /* @__PURE__ */ React__namespace.createElement(Text, {
    variant: "sans-12"
  }, repositoryName))), /* @__PURE__ */ React__namespace.createElement("div", {
    className: "grid gap-2 pl-5 pt-5 grid-flow-col"
  }, /* @__PURE__ */ React__namespace.createElement("dt", null, /* @__PURE__ */ React__namespace.createElement(Text, {
    variant: "sans-12",
    className: "font-semibold"
  }, "Plugin Version")), /* @__PURE__ */ React__namespace.createElement("dd", null, /* @__PURE__ */ React__namespace.createElement(Text, {
    variant: "sans-12"
  }, VERSION))))));
};
...
const ModalLoading = ({
  ...
  return /* @__PURE__ */ React__namespace.createElement(Modal, {
    ...
    variant: "sans-24",
    className: "text-slate-10 text-center font-semibold"
  }, "Fetching preview"), /* @__PURE__ */ React__namespace.createElement(Text, {
    variant: "sans-12-14",
    className: "text-center"
  }, "Please wait while your updates are loading\u2026")), /* @__PURE__ */ React__namespace.createElement(Button, {
    variant: "white",
    onClick: onDismiss,
    className: "mx-auto"
  }, /* @__PURE__ */ React__namespace.createElement(Text, {
...
const PrismicPreviewUI = ({
  ...
  const goToIdle = () => dispatch({ type: exports.PrismicContextActionType.GoToIdle });
  return /* @__PURE__ */ React__namespace.createElement(React__namespace.Fragment, null, state.activeRepositoryName && /* @__PURE__ */ React__namespace.createElement(Root, null, /* @__PURE__ */ React__namespace.createElement(ModalLoading, {
    isOpen: state.previewState === exports.PrismicPreviewState.BOOTSTRAPPING || state.previewState === exports.PrismicPreviewState.RESOLVING,
    repositoryName: state.activeRepositoryName,
    onDismiss: goToIdle
  }), /* @__PURE__ */ React__namespace.createElement(ModalAccessToken, {
    isOpen: state.previewState === exports.PrismicPreviewState.PROMPT_FOR_ACCESS_TOKEN,
    repositoryName: state.activeRepositoryName,
    ...
    onDismiss: goToIdle
  }), /* @__PURE__ */ React__namespace.createElement(ModalError, {
    isOpen: state.previewState === exports.PrismicPreviewState.FAILED,
    repositoryName: state.activeRepositoryName,
...
const withPrismicPreview = (WrappedComponent, repositoryConfigs = [], config = {}) => {
  const WithPrismicPreview = (props) => {
    ...
    const afterAccessTokenSet = React__namespace.useCallback(() => {
      contextDispatch({ type: exports.PrismicContextActionType.GoToIdle });
      bootstrapPreview();
    }, [bootstrapPreview, contextDispatch]);
    React__namespace.useEffect(() => {
      bootstrapPreview();
    }, [bootstrapPreview]);
    return /* @__PURE__ */ React__namespace.createElement(React__namespace.Fragment, null, /* @__PURE__ */ React__namespace.createElement(WrappedComponent, {
      ...props,
      data: mergedData.data,
      isPrismicPreview: mergedData.isPreview,
      prismicPreviewOriginalData: props.data
    }), /* @__PURE__ */ React__namespace.createElement(PrismicPreviewUI, {
      afterAccessTokenSet
    }));
  };
...
const withPrismicPreviewResolver = (WrappedComponent, repositoryConfigs = [], config = {}) => {
  const WithPrismicPreviewResolver = (props) => {
    ...
    const isPreview = contextState.previewState === exports.PrismicPreviewState.IDLE ? null : contextState.previewState !== exports.PrismicPreviewState.NOT_PREVIEW;
    const afterAccessTokenSet = React__namespace.useCallback(() => {
      contextDispatch({ type: exports.PrismicContextActionType.GoToIdle });
      resolvePreview();
    ...
    React__namespace.useEffect(() => {
      ...
      if (contextState.resolvedPath && ((_a = config.autoRedirect) != null ? _a : true)) {
        const navigate = config.navigate || gatsby__namespace.navigate;
...
const camelCase = (...parts) => cc__namespace.camelCase(parts.filter((p) => p != null).join(" "), {
  transform: cc__namespace.camelCaseTransformMerge
});
...
const useNodesForPath = (path) => {
  const [state, setState] = React__namespace.useState(0);
...
const useActiveRuntime = () => {
  ...
  return React__namespace.useMemo(() => contextState.activeRepositoryName ? contextState.runtimeStore[contextState.activeRepositoryName] : void 0, [contextState.activeRepositoryName, contextState.runtimeStore]);
};
const useActiveRepositoryConfig = (repositoryConfigs = []) => {
  const [contextState] = usePrismicPreviewContext();
  return React__namespace.useMemo(() => [...repositoryConfigs, ...contextState.repositoryConfigs].find((config) => config.repositoryName === contextState.activeRepositoryName), [
...
const withPrismicUnpublishedPreview = (WrappedComponent, repositoryConfigs, config = {}) => {
  const WithPrismicUnpublishedPreview = (props) => {
    ...
    const ResolvedComponent = React__namespace.useMemo(() => {
      var _a;
    ...
    const resolvedData = React__namespace.useMemo(() => {
    ...
    return contextState.previewState === exports.PrismicPreviewState.ACTIVE ? /* @__PURE__ */ React__namespace.createElement(ResolvedComponent, {
      ...props,
      data: resolvedData
    }) : /* @__PURE__ */ React__namespace.createElement(React__namespace.Fragment, null, /* @__PURE__ */ React__namespace.createElement(WrappedComponent, {
      ...props
    }), /* @__PURE__ */ React__namespace.createElement(PrismicPreviewUI, {
      afterAccessTokenSet
    }));
...
exports.PrismicPreviewProvider = PrismicPreviewProvider;
exports.componentResolverFromMap = componentResolverFromMap;
exports.contextReducer = contextReducer;
exports.defaultDataResolver = defaultDataResolver;
exports.useMergePrismicPreviewData = useMergePrismicPreviewData;
exports.usePrismicPreviewAccessToken = usePrismicPreviewAccessToken;
exports.usePrismicPreviewBootstrap = usePrismicPreviewBootstrap;
exports.usePrismicPreviewContext = usePrismicPreviewContext;
